...
$uncleanmsg = file_get_contents('php://input');
		...
		if(strpos($uncleanmsg,'published') !== false){
			...
			$_sendername = $parsevar['entry'][0]['changes'][0]['value']['sender_name'];
			$x_post_id = $parsevar['entry'][0]['changes'][0]['value']['post_id'];
			$x_verb = $parsevar['entry'][0]['changes'][0]['value']['verb'];
			$x_published = $parsevar['entry'][0]['changes'][0]['value']['published'];
			...
			if(isset($parsevar['entry'][0]['messaging'][0]['message']['mid'])){
				...
				if ($x_page_id == $x_sender_id){
					...
					$ins = "insert into fb_private_message set fbfrom_id='".$x_sender_id."', message='".str_replace("'", "", $x_text) ."', message_id='". $x_mid ."', created_time=from_unixtime('".$x_time."'), fbto_id='". $x_recipient_id ."', object_id='". $x_seq ."', fb_page_id='". $x_page_id ."', stat='MY COMMENT', sticker_id='".number_format($x_sticker_id,0,'','')."'";
					//$ins = "insert into fb_private_message set fbfrom_id='".$x_sender_id."', message='".str_replace("'", "",$x_text.)"', message_id='". $x_mid ."', created_time=from_unixtime('".$x_time."'), fbto_id='". $x_recipient_id ."', object_id='". $x_seq ."', fb_page_id='". $x_page_id ."', stat='MY COMMENT', sticker_id='".number_format($x_sticker_id,0,'','')."'";
					...
					if($x_text == "no message included"){
						$ins = "insert into fb_private_message set fbfrom_id='".$x_sender_id."', message='".str_replace("'", "", $x_text) ."', message_id='". $x_mid ."', created_time=from_unixtime('".$x_time."'), fbto_id='". $x_recipient_id ."', object_id='". $x_seq ."', fb_page_id='". $x_page_id ."', stat='PENDING', sticker_id='".number_format($x_sticker_id,0,'','')."'";
					}else{
					...
					if($parsevar['entry'][0]['changes'][0]['value']['item'] == "comment"){
						...
						$chk = "select id from fb_mypost_comment where id='".$x_comment_id."' and fb_page_id='". $x_page_id ."'";
							...
							if($x_page_id == $x_sender_id){
								...
								$ins = "insert into fb_mypost_comment set id='".$x_comment_id."', post_id='".$x_post_id."', aid='".$x_sender_name."', fromid='".$x_sender_id."', post_fbid='".$x_post_id."', parent_id='".$x_parent_id."', time='".$x_created_time."', text='".$x_message."', fb_page_id='".$x_page_id."', status='PENDING'";
